...
- `internal/notify/` — per-instance outbound notifications: the user-described HTTP request sent when one of that instance's backups succeeds or fails
...
- A notification is described entirely by the user — method, endpoint, body, and the body field the controller writes its message into. Body and message field are both optional, but a body that is given must be a JSON object whether or not the message field names a place inside it — validating it only when something is written into it made a trailing comma save fine and then fail the moment the field was filled in. With no message field the body is sent byte for byte, preserving key order; with neither one the request carries no body at all. `on_success` fires for COMPLETED; `on_failure` fires for FAILED *and* INCOMPLETE, since an interrupted backup left artifacts nobody asked for. It is sent from a defer in `ExecuteBackup`, so every terminal path notifies exactly once, including the ones that fail before a component runs; the defer is registered first so it runs last, after the last-backup status is persisted and the backup slot is released — a slow webhook must not hold the slot or make the post-backup sweep think a backup is still in flight. A notification that cannot be delivered is logged and dropped — the backup is already finished and its recorded result never depends on the endpoint answering
